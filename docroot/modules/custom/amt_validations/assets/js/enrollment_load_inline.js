(function ($, Drupal) {
    ...
    $(document).ajaxComplete(function () {
      ...
      if (!initialized2 && $(wrapper).length) {
        ...
        if (valueofAccount && !NewelementValue) {
          LoadEnrollmentContent(EnrollmentListURL, valueofAccount, createID, enrollmentfield);
        }


        // fill newly added element (select) with options from json based on student ID
        $(document).on('change', studentfield, function () {
          LoadEnrollmentContent(EnrollmentListURL, $(this).val(), createID, false);
        });
        ...
        $(document).on('change', '#' + createID, function () {
          ...
          var text = document.getElementById(createID).options[document.getElementById(createID).selectedIndex].innerHTML;
          $(enrollmentfield).val(text + ' (' + enrollmentValue + ')');
          var lessontype = $('select[data-drupal-selector="edit-field-type"]').val();
          var studentfieldvv = $(studentfield).val().split(" ").pop().replace(/\D/g, '');
          var ErrorDataURL = EnrollmentCheckURL + lessontype + '/' + studentfieldvv + '/' + enrollmentValue;
  ...
  /**
   * @function 
   * Load Enrollment Select Options form URL by Ajax.
   *
   * EnrollmentListURL @param
   *   URL of List (Enrollment List).
   * valueofAccount @param
   *   Value of Student Account.
   * createID @param
   *   ID of the Newly Created Element. (or Main Select).
   * enrollmentField @param
   *   Field of Enrollment.
   */
  function LoadEnrollmentContent(EnrollmentListURL, valueofAccount, createID, enrollmentfield) {
     // Split the values with a space.
    ...
    if(!valueofAccount) {
      ...
      $.getJSON(EnrollmentListURL + valueofAccount + '/' + window.location.pathname.indexOf("events") , function (data) {
        if (data === undefined || data.length == 0) {
          // if have data add empty Options.
